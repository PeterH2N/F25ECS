...
public class CollisionHelper
{
    public static boolean checkCollision(Manifold m) {
        ...
        return switch (m.aShape) {
            ...
            case AABBShape s -> switch (m.bShape) {
                ...
                case OBShape c -> AABBvsOB(m);
                default -> throw new IllegalStateException("Unexpected value: " + m.bShape);
            };
            case OBShape s -> switch (m.bShape) {
                case OvalShape c -> OBvsOval(m);
                case AABBShape c -> OBvsAABB(m);
                case OBShape c -> OBvsOB(m);
                default -> throw new IllegalStateException("Unexpected value: " + m.bShape);
            };
    ...
    private static boolean AABBvsOB(Manifold m) {
        AABBShape ab = (AABBShape) m.aShape;
        OBShape ob = (OBShape) m.bShape;
        Vector2D aPos = m.aPos.position.getAdded(m.aOffset);
        Vector2D bPos = m.bPos.position.getAdded(m.bOffset);
    ...
    private static boolean OBvsAABB(Manifold m) {
        Node e = m.a;
        m.a = m.b;
        m.b = e;
    ...
    private static boolean OBvsOval(Manifold m) {
        OBShape ob = (OBShape) m.aShape;
        OvalShape ov = (OvalShape) m.bShape;
        Vector2D aPos = m.aPos.position.getAdded(m.aOffset);
        Vector2D bPos = m.bPos.position.getAdded(m.bOffset);
        ...
        double xExtent = ob.width / 2;
        double yExtent = ob.height / 2;

        closest.x = Math.clamp(closest.x, -xExtent, xExtent);
        closest.y = Math.clamp(closest.y, -yExtent, yExtent);

        boolean inside = n.equals(closest);
        ...
        // collision normal needs to be flipped to point outside if circle was inside the OB
        if (inside) {
            m.normal = normal.getMultiplied(-1);
            m.penetration = d+r;
        }
    ...
    private static boolean OvalvsOB(Manifold m) {
        Node e = m.a;
        m.a = m.b;
        m.b = e;
    ...
    private static boolean OBvsOB(Manifold m) {
        m.normal = new Vector2D(0,0);
        m.penetration = 0;
        m.collide = false;
        return false;
    }
